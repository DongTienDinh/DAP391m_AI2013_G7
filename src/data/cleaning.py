...
from src.data.schema import OLIST_SCHEMAS
...
class DataService:
        ...
        for key, schema in OLIST_SCHEMAS.items():
            ...
            if len(df) < schema.min_rows:
                raise DataValidationError(
                    f"Dataset '{key}' has only {len(df)} rows (min expected: {schema.min_rows})"
                )
    ...
    def _transform(self, datasets: dict[str, pd.DataFrame]) -> dict[str, pd.DataFrame]:
        ...
        reviews = datasets["order_reviews"].drop(
            columns=["review_comment_title", "review_comment_message"], errors="ignore"
        )
