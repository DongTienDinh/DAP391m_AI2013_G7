...
class OlistPipeline:
    ...
    def run_full_pipeline(self) -> None:
        ...
        data_svc = DataService(self.paths.raw_olist, self.paths.processed_olist)
        data_svc.run_cleaning_pipeline()
        
        # 2. Feature Engineering
        feat_svc = FeatureEngineeringService(
            self.paths.processed_olist, 
            self.paths.processed_olist / "features_weekly.csv",
            self.paths.processed_olist / "prediction_data.csv"
        )
        feat_svc.run_feature_pipeline(self.paths.external_pop, self.paths.external_gdp)
        
        # 3. Model Training & Benchmarking
        model_svc = ModelingService(self.config.training.model_dump(), self.paths.reports.figures_dir.parent)
        model_svc.run_training_pipeline(self.paths.processed_olist / "features_weekly.csv")
